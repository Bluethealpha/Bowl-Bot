...
class Randimals:
    ...
    async def _dog(self, ctx: commands.Context):
        ...
        await self.bot.type()
        url = "https://giphy.com/gifs/corgi-sit-teach-how-to-H2ZHF0zIrCJLG"
    ...
    @commands.command(pass_context=True, no_pm=True, name="smoltoko")
async def _smoltoko(self, ctx: commands.Context):
    """Shows a random smol."""
        
        await self.bot.type()
        url = "https://giphy.com/tv/search/corgi"
        async with aiohttp.get(url + "woof") as response:
            filename = await response.text()
            async with aiohttp.get(url + filename) as image:
                await self.bot.upload(
